...
describe("npm launcher", () => {
  ...
  test("maps windows x64 to the windows scoped package", () => {
    expect(platformPackageName({ platform: "win32", arch: "x64" })).toBe("@agent-switch/windows-x64");
  });
  ...
  test("builds windows binary paths with exe suffix", () => {
    expect(binaryPathForCommand("/tmp/pkg", "agent-switch", true)).toBe("/tmp/pkg/agent-switch.exe");
  ...
  test("runCommand resolves the platform package and forwards argv", () => {
    ...
    const status = runCommand("as", {
      ...
      requireFn: {
        resolve: (specifier: string) => {
          expect(specifier).toBe("@agent-switch/windows-x64/package.json");
          return "/tmp/node_modules/@agent-switch/windows-x64/package.json";
        },
      },
    ...
    expect(calls).toEqual([
      {
        binaryPath: "/tmp/node_modules/@agent-switch/windows-x64/as.exe",
        argv: ["doctor", "--json"],
